...
eval_client.start_new_job(
    run_id="run_2026_02_03",
    config_path=[
        "ebench/teleop/frame_against_pen_holder"
    ],
)
...
try:
    ...
    while True:
        ...
        if left_gripper < 0.005:
            left_gripper = [0., 0.]  # close gripper
        else:
            left_gripper = [left_gripper, left_gripper] 

        if right_gripper < 0.005:
            right_gripper = [0., 0.]  # close gripper
        else:
            right_gripper = [right_gripper, right_gripper] 
        ...
        # obs['0']['obs']['state.joints'] = action[:12]
        # obs['0']['obs']['camera_data']['top_camera']['rgb'] = \
        #     cv2.cvtColor(obs['0']['obs']['camera_data']['top_camera']['rgb'], cv2.COLOR_BGR2RGB)
        # obs['0']['obs']['camera_data']['left_camera']['rgb'] = \
        #     cv2.cvtColor(obs['0']['obs']['camera_data']['left_camera']['rgb'], cv2.COLOR_BGR2RGB)
        # obs['0']['obs']['camera_data']['right_camera']['rgb'] = \
        #     cv2.cvtColor(obs['0']['obs']['camera_data']['right_camera']['rgb'], cv2.COLOR_BGR2RGB)
